...
defined('_JEXEC') || die('=;)');
$db = JFactory::getDbo();
$query = "SELECT count(*) FROM #__sa_tranzactii WHERE `aprobata` = '1'";
$db->setQuery($query);
...
$query = "SELECT * FROM #__sa_tranzactii WHERE `aprobata` = '1' ORDER BY `id` DESC LIMIT ".$inceput.", ".$rezult_pagina." ";
$db->setQuery($query);
	...
	echo '<tr class="'.$style.'">';
		...
		echo '<td class="sa_table_data">';
			if ($l->fisier == '') {
				...
				//afisare fisier
				$path = JUri::root().'administrator'.DS.'components'.DS.'com_sauto'.DS.'assets'.DS.'files'.DS;
				//echo $path;
