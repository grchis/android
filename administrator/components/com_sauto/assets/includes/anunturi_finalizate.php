...
$query = "SELECT count(*) FROM #__sa_anunturi WHERE `proprietar` = '".$uid."' AND `uid_winner` != '0'";
$db->setQuery($query);
$total = $db->loadResult();
...
$link = JRoute::_('index.php?option=com_sauto&task=anunturi_finalizate');
...
$query = "SELECT `a`.`titlu_anunt`, `a`.`anunt`, `a`.`id`, `a`.`raportat`, `a`.`data_adaugarii`, `a`.`is_winner`, `a`.`uid_winner`, `p`.`fullname`, `a`.`tip_anunt`, `p`.`uid` FROM #__sa_anunturi AS `a` JOIN #__sa_profiles AS `p` ON `a`.`proprietar` = '".$uid."' AND `a`.`uid_winner` != '0' AND `a`.`proprietar` = `p`.`uid` ORDER BY `a`.`id` DESC LIMIT ".$inceput.", ".$rezult_pagina." ";
$db->setQuery($query);
$list = $db->loadObjectList();
$image_path = JUri::root().'components/com_sauto/assets/';
?>
<h2>Anunturi finalizate</h2>
