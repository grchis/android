...
$db = JFactory::getDbo();
$query = "SELECT count(*) FROM #__sa_model_auto WHERE `mid` = '".$id."'";
$db->setQuery($query);
$total = $db->loadResult();
...
$link = JRoute::_('index.php?option=com_sauto&task=setari&action=lista_modele');
...
$query = "SELECT * FROM #__sa_model_auto WHERE `mid` = '".$id."' ORDER BY `id` DESC LIMIT ".$inceput.", ".$rezult_pagina." ";
$db->setQuery($query);
$list = $db->loadObjectList();
//get marca auto
$query = "SELECT `marca_auto` FROM #__sa_marca_auto WHERE `id` = '".$id."'";
$db->setQuery($query);
$marca_a = $db->loadResult();
?>
<h3>Modele ale marcii <?php echo $marca_a; ?></h3>
...
foreach ($list as $l) {
	if ($style == ' sa-row1 ') { 
			...
			$return_link = 'index.php?option=com_sauto&task=setari&action=lista_modele&id='.$id;
			echo '<tr class="'.$style.'">';
