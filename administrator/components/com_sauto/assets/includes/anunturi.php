...
if ($filtrat == 0) {
	...
	$string = $app->getUserState('string');
	$query = "SELECT count(*) FROM #__sa_anunturi WHERE `titlu_anunt` LIKE '%".$string."%'";
}
$db->setQuery($query);
$total = $db->loadResult();
...
$link = JRoute::_('index.php?option=com_sauto&task=anunturi');
...
if ($filtrat == 0) {
	$query = "SELECT `a`.`titlu_anunt`, `a`.`anunt`, `a`.`id`, `a`.`raportat`, `a`.`data_adaugarii`, `a`.`is_winner`, `a`.`uid_winner`, 
				`p`.`fullname`, `a`.`tip_anunt`, `p`.`uid` 
				FROM #__sa_anunturi AS `a` 
				JOIN #__sa_profiles AS `p` 
				ON `a`.`proprietar` = `p`.`uid` 
				ORDER BY `a`.`id` DESC LIMIT ".$inceput.", ".$rezult_pagina." ";
} else {
	$query = "SELECT `a`.`titlu_anunt`, `a`.`anunt`, `a`.`id`, `a`.`raportat`, `a`.`data_adaugarii`, `a`.`is_winner`, `a`.`uid_winner`, 
				`p`.`fullname`, `a`.`tip_anunt`, `p`.`uid` 
				FROM #__sa_anunturi AS `a` 
...
?>
<h2>Toate anunturile</h2>
<div>
<?php $link_filter = 'index.php?option=com_sauto&task=set_vars'; ?>
<h3>Filtrare anunturi</h3>
<form action="<?php echo $link_filter; ?>" method="post">
	<input type="hidden" name="type" value="anunt" />
	Titlu anunt <input type="text" name="string" value="<?php echo $string; ?>" /> 
