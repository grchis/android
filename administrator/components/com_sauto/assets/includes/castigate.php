...
$link = JRoute::_('index.php?option=com_sauto&task=castigate');
...
$query = "SELECT `a`.`titlu_anunt`, `a`.`anunt`, `a`.`id`, `a`.`raportat`, `a`.`data_adaugarii`, `a`.`is_winner`, `a`.`uid_winner`, `p`.`fullname`, `a`.`tip_anunt`, `p`.`uid` FROM #__sa_anunturi AS `a` JOIN #__sa_profiles AS `p` ON `a`.`uid_winner` = '".$uid."' AND `a`.`proprietar` = `p`.`uid` ORDER BY `a`.`id` DESC LIMIT ".$inceput.", ".$rezult_pagina." ";
$db->setQuery($query);
