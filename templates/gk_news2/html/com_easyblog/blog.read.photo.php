...
<header>
		...
		<div class="itemBody <?php echo $class; ?>">
		<div class="blog-text clearfix prel mtm mbm">
			...
			<?php if( $blog->images ){ ?>
				<?php foreach( $blog->images as $image ){ ?>
							...
							<a href="<?php echo $image;?>" target="_blank" class="easyblog-thumb-preview"><img src="<?php echo $image;?>" /></a>
					</p>
				<?php } ?>
			<?php } ?>
	...
	<!-- Load social buttons -->
		<?php if( in_array( $system->config->get( 'main_socialbutton_position' ) , array( 'top' , 'left' , 'right', 'bottom' ) ) || ( $system->config->get( 'main_ratings' )) || ( $this->getParam( 'show_hits' , true )) ){ ?>
