...
if ($total == 0) {
	...
	<div class="sa_missing_request">
		...
		<div class="sa_missing_request_right">
		<?php 
		$link_all = JRoute::_('index.php?option=com_sauto&view=all_comments');
		echo '<a href="'.$link_all.'">'.JText::_('SAUTO_READ_ALL_COMMENTS').'</a>'; ?>
		</div>
	</div>
	</div>
...
$query = "SELECT `p`.`fullname`, `p`.`poza`,  `p`.`calificative`, `c`.`proprietar`, `c`.`data_adaugarii`, `c`.`mesaj`, `c`.`anunt_id`, `a`.`titlu_anunt` FROM #__sa_comentarii as `c` JOIN #__sa_anunturi as `a` JOIN #__sa_profiles as `p` ON `c`.`companie` = '".$uid."' AND `c`.`raspuns` = '0' AND `c`.`anunt_id` = `a`.`id` AND `c`.`proprietar` = `p`.`uid` ORDER BY `c`.`id` DESC LIMIT ".$inceput.", ".$rezult_pagina." ";
...
foreach ($list as $l) {
	...
	echo '<td class="sa_table_cell" valign="top" width="100">';
		if ($l->poza != '') {
				$poza = $image_path.$l->proprietar."/".$l->poza;
			} else {
