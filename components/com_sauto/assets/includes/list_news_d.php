...
$query = "SELECT `r`.`anunt_id`, `r`.`proprietar`, `r`.`mesaj`, `r`.`pret_oferit`, `r`.`moneda`, `a`.`titlu_anunt`, `p`.`fullname`, `p`.`poza`, `m`.`m_scurt` FROM #__sa_raspunsuri AS `r` JOIN #__sa_anunturi AS `a` JOIN #__sa_profiles AS `p` JOIN #__sa_moneda AS `m` ON  `r`.`anunt_id` = `a`.`id` AND `r`.`proprietar` = `p`.`uid` AND `r`.`moneda` = `m`.`id` AND `r`.`firma` = '".$uid."' AND `r`.`status_raspuns` = '1' ORDER BY `r`.`id` DESC LIMIT 0, 5 ";
$db->setQuery($query);
$list = $db->loadObjectList();
$i=1;
?>
<h3><?php echo JText::_('SA_ULTIMELE_OFERTE_CASTIGATE'); ?></h3>

<?php

if (empty($list)) {
	echo JText::_('SAUTO_NU_SUNT_OFERTE_CASTIGATE');
} else {
	echo '<div id="sa_viz_main">';
	foreach ($list as $l) {
		if ($class == ' sa-row1 ') { 
			$class = ' sa-row0 '; 
		} else { 
