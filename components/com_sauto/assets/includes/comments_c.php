...
$query = "SELECT `c`.`id`, `c`.`readed_c`, `p`.`companie`, `p`.`poza`,  `p`.`calificative`, `c`.`companie` as `comp`, `c`.`data_adaugarii`, `c`.`mesaj`, `c`.`anunt_id`, `a`.`titlu_anunt` 
FROM #__sa_comentarii as `c` JOIN #__sa_anunturi as `a` JOIN #__sa_profiles as `p` 
ON `c`.`proprietar` = '".$uid."' AND `c`.`raspuns` = '1' AND `c`.`anunt_id` = `a`.`id` AND `c`.`companie` = `p`.`uid` ORDER BY `c`.`id` DESC LIMIT ".$inceput.", ".$rezult_pagina." ";
$db->setQuery($query);
...
foreach ($list as $l) {
	...
	echo '<td class="sa_table_cell" valign="top">';
		...
		echo '<hr />';
		echo '<div>';
			if ($l->readed_c == 0) { echo '<img src="'.$img_path.'icon_nou.png" /> '; }
