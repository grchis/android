...
<div>
	...
	<div class="sauto_form_label">		<?php echo JText::_('SAUTO_TIP_TRANSMISIE'); ?>		<?php echo ' <span class="sa_obligatoriu">'.JText::_('SA_ASTERISC').'</span>'; ?>	</div>	<div>		<select name="transmisie" class="sa_select">			<option value=""><?php echo JText::_('SAUTO_ALEGE_TRANSMISIE'); ?></option>			<?php			$query = "SELECT * FROM #__sa_transmisie WHERE `published` = '1'  ORDER BY `transmisie` ASC";			$db->setQuery($query);			$transm = $db->loadObjectList();			foreach ($transm as $trm) {				echo '<option value="'.$trm->id.'"';					if ($trm->id == $rezult->transmisie) { echo ' selected'; }				echo '>'.$trm->transmisie.'</option>';			}			?>		</select>	</div>	
	<div class="sauto_form_label">
	...
	*/ 
	...
	<div class="sauto_form_label">
		<?php echo JText::_('SAUTO_ANUNT'); ?>
			...
			<div>
			<textarea name="anunt8" id="anunt8" cols="60" rows="20" style="width: 500px; height: 150px;" class="mce_editable" aria-hidden="true">
				<?php
					echo $rezult->anunt;
				?>
			</textarea>
