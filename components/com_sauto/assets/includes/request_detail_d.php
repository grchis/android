...
<!---=========================OFERTE===================-->	
	...
	<?php
			...
			if ($total != 0) {
				//avem poze
				$content=file_get_contents('display_pictures.php');
				echo $content;	
				view_pictures($id, $rezult->proprietar);
			}
			?>
...
echo '<div><div class="report" style="float:right!important;">';
report_now($rezult->raportat, $id, $uid);
echo '</div><br>';
					?>
					<div style="float:left;" onClick="toggle_visibility('add_price');" class="sa_table_cell sa_phone sa_min_width_offer sa_cursor sa_hover">
						<p class="sa_oferte_span">
						<?php 
					 echo JText::_('SAUTO_DEALER_MAKE_OFFER'); ?>
						</p>
					</div>	
...
$link_add_price = JRoute::_('index.php?option=com_sauto&view=make_offer'); 
	...
	<?php echo JText::_('SAUTO_MESAJ_OFERTA'); ?>
		<textarea cols="45" rows="5" name="mesaj"></textarea>
		<?php echo JText::_('SAUTO_PRET_OFERTA'); ?><br/>
								<input type="text" name="pret" value="" />
								<select name="moneda">
								<?php
								$query = "SELECT * FROM #__sa_moneda WHERE `published` = '1'";
		...
		<?php
								...
								?>
			<div style="float:right; class="sa_table_cell sa_phone sa_min_width_offer sa_cursor sa_hover" onClick="document.forms['sa_send_offer'].submit();">
									<span class="sa_oferte_span">
										<?php echo JText::_('SAUTO_DEALER_MAKE_THIS_OFFER'); ?>
									</span>
								</div>
...
############				
foreach ($rasps as $r) {
	echo '<div class = "main-container" style="padding-top: 23px;">
		<div id="request-item" style="border-bottom: 1px solid #509EFF; padding-bottom: 10px;">';
	//if ($uid == $r->firma) {
		
	$link_profile = JRoute::_(
	'index.php?option=com_sauto&view=public_profile&id='.$r->firma);
		if ($r->poza == '') {
				//fara avatar
				echo '<img src="'.$img_path.'fi_avatar.png" width="18%" height="100px" style="vertical-align: top;" />';
			} else {
				//cu avatar
				echo '<img src="'.$img_path2.$r->firma.'/'.$r->poza.'" width="18%" height="100px" style="vertical-align: top;" />';
			}
			echo '<div style="display: inline-block;  width: 65%;">';
			//detalii 
			echo '<p class="request-p" style="font-size: 21px; margin: 0 !important; padding-top: 0px;"><a class="sa_public_profile" href="'.$link_profile.'">'.$r->companie.'</a></p>';
			...
			echo '</div>';
			echo '<p class="sa_tip_abon">'.JText::_('SAUTO_TIP_VANZATOR').': '.$r->abonament.'</p>';
	#######################rand2
		echo '<div>'.JText::_('SAUTO_MESAJ_CERERE_TITLE').':</div>';
			echo '<div>'.$r->mesaj.'</div>';
				$data = explode(" ", $r->data_adaugarii);
			...
			echo '</div><div>';
			echo '<div class="sa_table_cell sa_phone sa_min_width_offer sa_hover">';
					echo '<p class="sa_oferte_span">';
					echo ' <p style="color:white;margin-left:15px;">'.$r->telefon.'</p>';
					echo '</p>';
				echo '</div>';	
