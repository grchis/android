		...
		if ($style == ' sa-row1 ') { 
			...
			?><tr class="sa_table_row <?php echo $style; ?>">
				...
				<td class="sa_table_cell" valign="top">
					...
					$query = "SELECT `p`.`telefon`,  `p`.`fullname`, `p`.`calificative`, `u`.`registerDate`, `j`.`judet` 
						FROM #__sa_profiles as `p` 
						JOIN  #__users as `u` 
						ON `p`.`uid` = '".$l->poster_id."' 
						JOIN #__sa_judete as `j` ON `p`.`judet` = `j`.`id`";
					$db->setQuery($query);
					$usr = $db->loadObject();
			...
			<tr class="sa_table_row <?php echo $style; ?>">
				<td class="sa_table_cell">
					<div class="sa_phone sa_min_width_offer sa_hover">
