...
<form action="<?php echo $link_form; ?>" method="post" enctype="multipart/form-data">
	...
	<div>
		...
		<input type="text" name="buget_min" value="" size="18" />
		<?php echo ' '.JText::_('SAUTO_SI').' '; ?>
		<input type="text" name="buget_max" value="" size="18" />
		<select name="buget_moneda">
			...
			foreach ($monede as $mons) {
				echo '<option value="'.$mons->id.'">'.$mons->m_scurt.'</option>';
			}
			?>
		</select>
	</div>		
	...
	<div>
			...
			echo $editor->display('anunt4', '', '500', '150', '60', '20', false);
			?>
			</div>
