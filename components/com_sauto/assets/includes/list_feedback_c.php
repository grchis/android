...
$query = "SELECT `c`.`anunt_id`, `c`.`poster_id`, `c`.`mesaj`, `c`.`tip`, `a`.`titlu_anunt`, `p`.`companie`, `p`.`calificative` FROM #__sa_calificativ AS `c` JOIN #__sa_anunturi AS `a` JOIN #__sa_profiles AS `p` ON  `c`.`anunt_id` = `a`.`id` AND `c`.`poster_id` = `p`.`uid` AND `c`.`dest_id` = '".$uid."' ORDER BY `c`.`id` DESC LIMIT 0, 5 ";
$db->setQuery($query);
