...
$query = "INSERT INTO #__sa_comentarii (`anunt_id`, `proprietar`, `companie`, `data_adaugarii`, `mesaj`, `published`, `ordonare`) VALUES ('".$anunt_id."', '".$proprietar."', '".$firma."', '".$publishDate."', '".$mesaj."', '1', '".$ordonare."')";
$db->setQuery($query);
$db->query();
$comm_id = $db->insertid();
###########################prelucrare imagine#############
...
###########################end prelucrare imagine##################
	...
	$query = "SELECT `p`.`reprezentant`, `p`.`alerte`, `u`.`email` FROM #__sa_profiles AS `p` JOIN #__users AS `u` ON `p`.`uid` = '".$firma."' AND `p`.`uid` = `u`.`id`";
	$db->setQuery($query);
	$rez = $db->loadObject();
	$alerte = explode(",", $rez->alerte);
	if (in_array("s-5_1", $alerte)) {
		$baza = JUri::base();
	...
	//print_r($rez);
//echo 'userul > '.$proprietar.' a comentat asta: <br />'.$mesaj.'<br /> la oferta lui '.$firma.' <br /> la anuntul > '.$anunt_id;
