...
$url_alert = $app->getUserState('url_alert');

if ($url_alert == 'profil') {
		$link_redirect = JRoute::_('index.php?option=com_sauto&view=edit_profile');
	} elseif ($url_alert == 'alerte') {
...
if ($alert_type == 'enable') {
	...
	if ($delete != 1) {
		$link_redirect2 = JRoute::_('index.php?option=com_sauto&view=edit_profile&task=alert_edit&id='.$alert_id);
