...
$query = "SELECT * FROM #__sa_anunturi WHERE `status_anunt` = '1' AND `is_winner` = '0' AND `tip_anunt` = '".$id."'
".$db_oferte." ".$db_piese." ".$db_judete." ".$db_marci." ".$db_orase." ".$db_modele."  ORDER BY `id` DESC LIMIT ".$inceput.", ".$rezult_pagina." ";
$db->setQuery($query);
...
<?php
	if ($id == '') {
		...
		$query = "SELECT `article_id` FROM #__sa_tip_anunt WHERE `id` = '".$id."' AND `published` = '1'";
		$db->setQuery($query);
...
if ($l->marca_auto != 0) {
	...
	echo '</div><div style="clear:both;"></div>';
			...
			echo '</div>';
			echo '<div class="sa_table_cell sa_phone sa_min_width_phone">';
			...
			echo '</td>';
		echo '</tr>';
		//echo '<tr class="sa_table_row '.$style.'"><td colspan="3" height="10"><hr /></td></tr>';
		if ($i == 5) {
			$style = 'sa-row0';
			echo '<tr class="sa_table_row '.$style.'">';
