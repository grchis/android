...
$query = "SELECT count(*) FROM #__sa_comentarii WHERE `ID` = '".$id."' AND `companie` = '".$uid."'";
$db->setQuery($query);
$total = $db->loadResult();
...
if ($total == 1) {
	...
	$r_id = '';
	//preiau comentariul
	$query = "SELECT `c`.`anunt_id`, `c`.`proprietar`, `c`.`data_adaugarii`, `c`.`mesaj`, `p`.`poza`, `p`.`fullname`, `p`.`calificative`, `a`.`titlu_anunt`, `a`.`anunt` FROM #__sa_comentarii AS `c` JOIN #__sa_profiles AS `p` JOIN #__sa_anunturi AS `a` ON `c`.`id` = '".$id."' AND `c`.`proprietar` = `p`.`uid` AND `c`.`anunt_id` = `a`.`id`";
	$db->setQuery($query);
	$list = $db->loadObject();
	//print_r($list);
	?>
<table class="sa_table_class">
	...
	<table width="100%"  class="sa_table_class">
		<tr class="sa_table_row">
			...
			<?php
			if ($list->poza != '') {
				$poza = $image_path.'users'.DS.$list->proprietar.DS.$list->poza;
			} else {
		...
		echo '<img src="'.$poza.'" width="80" />';
			...
			<td class="sa_table_cell" valign="top">
				<?php
				$link_profile = JRoute::_('index.php?option=com_sauto&view=public_profile&id='.$list->proprietar);
				$link_anunt = JRoute::_('index.php?option=com_sauto&view=request_detail&id='.$list->anunt_id);
				echo '<div><a class="sa_public_profile" href="'.$link_profile.'">'.$list->fullname.'</a></div>';
				echo '<hr />';
		...
		if ($pcs != 0) {
			...
			<tr class="sa_table_row">
				...
				$pics = $db->loadObjectList();
					foreach ($pics as $p) {
						$pic = $image_path.'users'.DS.$list->proprietar.DS.$p->poza;
		...
		<tr class="sa_table_row">
			<td class="sa_table_cell" colspan="3">
				...
				<?php
				form_comment($r_id, $multiple_id, $list->anunt_id, $list->proprietar, $uid);
	...
	$query = "UPDATE #__sa_comentarii SET `readed_d` = '1' WHERE `id` = '".$id."'";
	$db->setQuery($query);
	$db->query();
} else {
