...
$uid = $user->id;
$query = "SELECT `p`.`poza`, `p`.`fullname`,  `p`.`adresa`, `p`.`telefon`, `p`.`uid`, `j`.`judet`, `u`.`registerDate`, `l`.`localitate` FROM #__sa_profiles as `p` JOIN #__sa_judete as`j` JOIN #__users as `u` JOIN #__sa_localitati as `l` ON `p`.`uid` = '".$uid."' AND `p`.`uid` = `u`.`id` AND `p`.`judet` = `j`.`id` AND `p`.`localitate` = `l`.`id`";
 
//$query = "SELECT `p`.*, `j`.`judet`, `u`.`registerDate` FROM #__sa_profiles as `p` LEFT JOIN #__sa_judete as`j` ON `p`.`judet` = `j`.`id` AND `p`.`uid` = '".$uid."' JOIN #__users as `u` ON `u`.`id` = `p`.`uid`";
$db->setQuery($query);
