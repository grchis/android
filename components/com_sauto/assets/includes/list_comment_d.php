...
$query = "SELECT `c`.`id`, `c`.`readed_d`, `c`.`anunt_id`, `c`.`proprietar` , `c`.`data_adaugarii`, `c`.`mesaj`, `p`.`poza`, `p`.`fullname`, `p`.`calificative`, `a`.`titlu_anunt` FROM #__sa_comentarii AS `c` JOIN #__sa_profiles AS `p` JOIN #__sa_anunturi AS `a` ON `c`.`proprietar` = `p`.`uid` AND `c`.`anunt_id` = `a`.`id` AND `c`.`companie` = '".$uid."' AND `c`.`raspuns` = '0'  ORDER BY `c`.`id` DESC LIMIT 0, ".$limit."";
$db->setQuery($query);
...
if (empty($list)) {
	...
	foreach ($list as $l) {
		...
		<div class="sa_list_img sa_tables_cell_high sa_tables_align_left <?php echo $class; ?>">
			<?php
			if ($l->poza != '') {
				$poza = $image_path.'users/'.$l->proprietar."/".$l->poza;
			} else {
